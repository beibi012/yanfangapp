				...
				<view style="height: 150upx;"></view>
					...
					<view class="" v-show="show1">
						...
						<view class="basic_container" style="border-top: 1px solid #ddd;">
							...
							<picker mode="selector" name="roleId":range="user" :value="index" @change="changeUser" >
								<view class="basic_picker">{{user[index]}}
						...
						<view class="basic_container">
							...
							<input class="basic_input" name="projectName" type="text" :value="option1.projectName" >
						</view>
						...
						<view class="basic_container">
							...
							<input class="basic_input" name="clientName" type="text" :value="option1.clientName" >
						</view>
						<view class="basic_container">
							<text class="basic_title">地址</text>
							<input class="basic_input" name="clientAddress" type="text" :value="option1.clientAddress" >
						</view>
						<view class="basic_container">
							<text class="basic_title">面积</text>
							<input class="basic_input" name="area" type="text" :value="option1.area" >
						</view>
						<view class="basic_container">
							<text class="basic_title">联系人</text>
							<input class="basic_input" name="clientContacts" type="text" :value="option1.clientContacts" >
						</view>
						<view class="basic_container">
							<text class="basic_title">联系电话</text>
							<input class="basic_input" name="clientNumber" type="text" :value="option1.clientNumber" >
						</view>
						<view class="basic_container">
						...
						<view class="basic_container">
							<text class="basic_title">姓名</text>
							<input class="basic_input" name="ownerName" type="text" :value="option1.ownerName" >
						</view>
						<view class="basic_container">
							<text class="basic_title">地址</text>
							<input class="basic_input" name="ownerAddress" type="text" :value="option1.ownerAddress" >
						</view>
						<view class="basic_container">
							<text class="basic_title">联系人</text>
							<input class="basic_input" name="ownerContacts" type="text" :value="option1.ownerContacts" >
						</view>
						<view class="basic_container">
							<text class="basic_title">联系电话</text>
							<input class="basic_input" name="ownerNumber" type="text" :value="option1.ownerNumber" >
						</view>
						<view class="basic_container">
							...
							<view class="basic_container2">
								...
								<view class="picture" >
									<view class="pic_container" v-for="(p,index) in pic[i]" >
										...
										<view class="del" @click="delPic(i,index)"><image class="del_img" src="/static/delete@2x.png" mode="widthFix"></image></view>
									</view>
					...
					<view class="" v-show="show3">
						...
						<view v-for="smap in smapArray" style="border-bottom: 1px solid #ddd;">
							<view class="basic_container">
								...
								<input class="basic_input" :name="'	itemSample'+smap" type="text" value="" >
							</view>
							<!-- <view class="basic_container">
						...
						<view class="basic_container">
							...
							<input class="basic_input" :name="'nameSample'+smap" type="text" value="" >
						</view>
						<view class="basic_container">
							<text class="basic_title">样品描述{{smap+1}}</text>
							<input class="basic_input" :name="'descriptionSample'+smap" type="text" value="" >
						</view>
	...
	export default{
		...
		methods:{
			...
			submitOption2:(e)=>{
				...
				for(let j=0;j<self.itemArray.length;j++){
					self.option2[j]={}
					for (let key in e.detail.value) {
						if(key.search(j)!==-1){
							if(key.search("Name")!==-1){
								self.option2[j].monitoringItem=e.detail.value[key]
							}
							if(key.search("Method")!==-1){
								self.option2[j].analyzeCondition=e.detail.value[key]
							}
							if(key.search("remark")!==-1){
								self.option2[j].remark=e.detail.value[key]
							}
						}
					}
					self.fileid[j]=""
					self.filepath[j]=""
					self.filenumber[j]=0
					for(let i=0;i< self.pic[j].length; i++){
					uni.uploadFile({
							...
							fileType: "image", 
					            ...
					            success: (res) => {
									...
									self.fileid[j]=self.fileid[j]+imgId;
									console.log("获取到nowFileId为:"+self.fileid[j]);
									
									let imgPath=JSON.parse(res.data).urls
									console.log("获取到imgPath为:"+imgPath);
									self.filepath[j]=self.filepath[j]+imgPath;
									console.log("获取到filepath为:"+self.filepath[j]);
								...
								complete:() => {
									if(self.filenumber[j]==self.pic[j].length){
										self.fileid[j]=self.fileid[j].substring(0,self.fileid[j].length-1);
										self.filepath[j]=self.filepath[j].substring(0,self.filepath[j].length-1);
										self.option2[j].nowFileId=self.fileid[j]
										self.option2[j].nowFilePath=self.filepath[j]
										self.option2[j].itemSerialNumber=j+1
										console.log(self.option2)
										console.log(JSON.stringify(self.option2))
										uni.setStorage({
										    key: 'option2',
										    data: self.option2,
										    success: function () {
			...
			submitOption3:(e)=>{
				...
				//获取仪器设备信息
				self.option3.homeEquipments=[]
				for (let i = 0; i < self.instArray.length; i++) {
					self.option3.homeEquipments[i]={}
					for (let key in e.detail.value) {
						if(key.search("Inst"+i)!==-1){
							if(key.search("name")!==-1){
								self.option3.homeEquipments[i].name=e.detail.value[key]
							}
							if(key.search("model")!==-1){
								self.option3.homeEquipments[i].type=e.detail.value[key]
							}
							if(key.search("number")!==-1){
								self.option3.homeEquipments[i].factoryCode=e.detail.value[key]
							}
							if(key.search("factor")!==-1){
								self.option3.homeEquipments[i].itemId=e.detail.value[key]
							}
							
						}
					}
					self.option3.homeEquipments[i].equSerialNumber=i+1
					console.log("=====")
				}
				
				//获取检测内容信息
				self.option3.homeMonitoringContents=[]
				for (let i = 0; i < self.smapArray.length; i++) {
					self.option3.homeMonitoringContents[i]={}
					for (let key in e.detail.value) {
						if(key.search("Sample"+i)!==-1){
							if(key.search("item")!==-1){
								self.option3.homeMonitoringContents[i].monitoringItem=e.detail.value[key]
							}
							if(key.search("name")!==-1){
								self.option3.homeMonitoringContents[i].sampleName=e.detail.value[key]
							}
							if(key.search("description")!==-1){
								self.option3.homeMonitoringContents[i].sampleDescription=e.detail.value[key]
							}
							
						}
					}
					self.option3.homeMonitoringContents[i].contentSerialNumber=i+1
					console.log("=====")
				}
				self.option3.monitoringResult=e.detail.value.monitoringResult
				self.option3.airProblem=e.detail.value.airProblem
				self.option3.monGovernTime=e.detail.value.governDate+","+e.detail.value.governTime+":00"
				console.log(JSON.stringify(self.option3))
				...
				console.log("=======================")
				// console.log(JSON.stringify(self.option1))
				// console.log(JSON.stringify(self.option2))
				// console.log(JSON.stringify(self.option3))
				const app = getApp()
				console.log(app.globalData.ctx)
				...
				uni.request({
					...
					data: {
						...
						"temperature": self.option1.temperature,
						"humidity": self.option1.humidity,
						"monitoringSource": self.option1.surveySource,

						
						//第二步数据
						"homeMonitoringItems":self.option2,
						//第三步数据
						"homeEquipments":self.option3.homeEquipments,
						"homeMonitoringContents": self.option3.homeMonitoringContents,
						"monitoringResult": self.option3.monitoringResult,
						"airProblem": self.option3.airProblem,
						"monGovernTime": self.option3.monGovernTime
						},
			...
			delPic:(i,index)=>{
				let j=self.index_array[i]-index;
				console.log(j)
				console.log(self.pic[i])
				for(let k=0; k<j;k++){
					self.pic[i][index+k]=self.pic[i][index+k+1];
					self.pic[i].splice(index+k+1,0)
				}
				self.pic[i].pop();
				self.index_array[i]--;
				self.pic.push([])
