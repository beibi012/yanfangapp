	...
	<view>  
				<form @submit="submit">
					...
					<view class="basic_container" style="border-top: 1px solid #ddd;">
						...
						<text class="basic_content">{{data.roleId==0?'个人':data.roleId==1?'开发商':'工装'}}</text>
						</view>
						<view class="basic_container" style="border-top: 1px solid #ddd;">
							<text class="basic_title" style="font-size: 35upx;font-weight: bold;color: #333;">项目名称</text>
							<text class="basic_content">{{data.projectName}}</text>
						</view>
						<view class="basic_container" style="border-top: 1px solid #ddd;">
							...
							<text class="basic_content">{{data.homeCustomers[0].name}}</text>
							</view>
							<view class="basic_container" style="border-top: 1px solid #ddd;">
								<text class="basic_title" style="font-size: 35upx;font-weight: bold;color: #333;">联系方式</text>
								<text class="basic_content">{{data.homeCustomers[0].phone}}</text>
							</view>
							<view class="basic_container" style="border-top: 1px solid #ddd;">
								<text class="basic_title" style="font-size: 35upx;font-weight: bold;color: #333;">地  址</text>
								<text class="basic_content">{{data.homeCustomers[0].address}}</text>
							</view>
							<view class="basic_container" style="border-top: 1px solid #ddd;">
								<text class="basic_title" style="font-size: 35upx;font-weight: bold;color: #333;">面  积</text>
								<text class="basic_content">155㎡</text>
							</view>
					<!-- <view class="basic_container" style="border-top: 1px solid #ddd;">
						<text class="basic_title" style="font-size: 35upx;font-weight: bold;color: #333;">用户名称</text>
				...
				<view class="basic_container2" style="border-top: 1px solid #ddd;">
					<text class="basic_title">治理过程</text>
					<view class="picture_govern">
						<view class="pic_cont" v-for="(p,index) in pic">
							...
							<view class="pic_describe">
								<textarea class="describe_input" cursor="0" :name="'remarkP'+index" value="" placeholder="输入描述" />
								</view>
						</view>
						<view class="pic_cont" v-for="(v,index) in vde" @touch="moveHandle">
	...
	export default{
		...
		methods:{
			submit:(e)=>{
					...
					for (let i = 0; i < self.pic.length; i++) {
						uni.uploadFile({
						       ...
						       fileType: "image", 
								...
								complete:()=>{
									if(self.pic.length==self.picupload){
										console.log(JSON.stringify(self.imgList))
										self.imgID=self.imgID.substring(0,self.imgID.length-1)
										self.imgPath=self.imgPath.substring(0,self.imgPath.length-1)
				...
				if(self.vde!==[]){
					console.log("有视频")
					for (let j = 0; j < self.vde.length; j++) {
						console.log(self.vde[j])
						uni.uploadFile({
				           url:app.globalData.ctx+ '/common/uploadFileUnify',
				           fileType: "video", 
				           	header:{
				...
				uni.showActionSheet({
				    ...
				    success: function (res) {
						...
						if(res.tapIndex==1){
							uni.chooseVideo({
								success:function(res){
									let format=res.tempFilePath.substr(res.tempFilePath.length-3,3)
									console.log(format)
									if(format!=="AVI"&&format!=="FLV"){
