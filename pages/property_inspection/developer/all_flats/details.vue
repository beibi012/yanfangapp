		...
		<view class="show_container">
			...
			<view  class="half">
						...
						<view class="newly_problem">
							<navigator class="newly_botton" :url="'take_notes?psId='+psId">新增专项检测</navigator>
							</view>
			...
			uni.request({
				...
				success: res => {
					...
					for (let i = 0; i < res.data.rows.length; i++) {
						let exist=false
						for (let j = 0; j < self.main.length; j++) {
							if(self.main[j]==res.data.rows[i].pdLoca){
								exist=true
							}
						}
						if(exist==false){
					...
					for (let k = 0; k < res.data.rows.length; k++) {
						let exist=false
						for (let l = 0; l < self.item.length; l++) {
							if(self.item[l]==res.data.rows[k].pdLocation){
								exist=true
							}
						}
						if(exist==false){
		...
		data(){
			...
			methods:{
				...
				changePerson:(e)=>{
					...
					self.index3=0
					self.getmain=[]
					console.log(self.person[e.detail.value])
					for (let i = 0; i < self.getData.length; i++) {
						 if(self.getData[i].pdRecorder==self.person[e.detail.value]){
